...
class OrderProvider extends ChangeNotifier {
    // Helper to detect order type for mixed carts
    String _detectOrderType(List<OrderItem> items) {
      final hasFood = items.any((item) => item.itemType == 'Food');
      final hasStore = items.any((item) => item.itemType == 'Store');
      if (hasFood && hasStore) return 'Mixed';
    ...
    try {
      ...
      for (final json in data as List) {
        ...
        final items = (json['order_items'] as List).map((item) => OrderItem(
          ...
          menuItemId: item['item_type'] == 'Food' ? item['product_id'] : null,
          storeItemId: item['item_type'] == 'Store' ? item['product_id'] : null,
          title: item['name'],
          quantity: item['quantity'],
  ...
  void _sendOrderStatusNotification(String orderId, OrderStatus newStatus) {
    ...
    switch (newStatus) {
      case OrderStatus.pending_payment:
        ...
        message = 'Waiting for payment confirmation for order #$orderId';
        break;
    ...
    if (orderIndex != -1) {
      ...
      if (itemIndex != -1) {
        ...
        updatedItems[itemIndex] = OrderItem(
          ...
          menuItemId: order.items[itemIndex].menuItemId,
          storeItemId: order.items[itemIndex].storeItemId,
          title: order.items[itemIndex].title,
          quantity: order.items[itemIndex].quantity,
          price: order.items[itemIndex].price,
          itemType: order.items[itemIndex].itemType,
          rating: rating,
          review: review,
        );
  ...
  Future<String> createOrder({
    ...
    try {
      ...
      // Build delivery address JSONB with instructions
      final deliveryAddressJson = deliveryAddress != null 
        ? {
      ...
      // Insert into Supabase orders table matching actual schema
      // Note: 'id' (UUID) and 'short_id' (ORD-xxx) are auto-generated by database
      final response = await Supabase.instance.client.from('orders').insert({
        ...
        'order_type': _detectOrderType(items),
        'placed_at': DateTime.now().toIso8601String(),
      }).select().single();

      final orderId = response['id'] as String;
      final shortId = response['short_id'] as String?;
      
      debugPrint('✅ Order created: $orderId (${shortId ?? 'no short_id'}) with status pending_payment');

      // Insert order items
      for (final item in items) {
        await Supabase.instance.client.from('order_items').insert({
          'order_id': orderId,
          'product_id': item.itemType == 'Food' ? item.menuItemId : null,
          'store_item_id': item.itemType == 'Store Item' ? item.storeItemId : null,
          'name': item.title,
          'quantity': item.quantity,
          'unit_price': item.price,
          'total_price': item.totalPrice,
      ...
      debugPrint('✅ Order items inserted for $orderId');
